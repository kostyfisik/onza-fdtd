...
def PrintFinalMessage ():
        ...
        print "\n>> .png pictures and performance_data.txt can be found in root directory.\n"
...
def ChangeConfig (config_file, input_parameters):
	...
	if len(input_parameters) == 5:
		...
		for i in 6, 4, 2, 1:
			...
			f = open('explore3D_' + '%d' % (q) + '.config', 'w')
		...
		for i in 6, 4, 2, 1:
			...
			content_change[q-1] = content_change[q-1].replace('length_y = ' + '%d' % (a), 'length_y = ' + '%d' % (a_change[q-1]))
			content_change[q-1] = content_change[q-1].replace('total_time_steps = ' + '%d' % (t), 'total_time_steps = ' + '%d' % (t_change[q-1]))
			f = open('explore2D_' + '%d' % (q) + '.config', 'w')
			f.write(content_change[q-1])
			f.close()
		return "explore2D"
		...
		for i in 6, 4, 2, 1:
			...
			content_change[q-1] = content_change[q-1].replace('length_x = ' + '%d' % (a), 'length_x = ' + '%d' % (a_change[q-1]))
			content_change[q-1] = content_change[q-1].replace('total_time_steps = ' + '%d' % (t), 'total_time_steps = ' + '%d' % (t_change[q-1]))
			f = open('explore1D_' + '%d' % (q) + '.config', 'w')
			f.write(content_change[q-1])
			f.close()
		return "explore1D"
	else:
...
def RunConfigs (result, path, nodes_number, max_proc_number, number, Nodes, Procs, Means, Acceleration, Efficiency, opt_n, opt_N, optimal_procs, optimal_nodes):
		for q in 1,2,3:
			if result == "explore3D":
				ParametricRunNodes(path, binary_file, 'explore3D_' + '%d' % (q) + '.config', nodes_number, max_proc_number, number, Nodes, Procs, Means)
			elif result == "explore2D":
				ParametricRunNodes(path, binary_file, 'explore2D_' + '%d' % (q) + '.config', nodes_number, max_proc_number, number, Nodes, Procs, Means)
			elif result == "explore1D":
				ParametricRunNodes(path, binary_file, 'explore1D_' + '%d' % (q) + '.config', nodes_number, max_proc_number, number, Nodes, Procs, Means)
			...
			if result == "explore3D":		
				WriteDataNodes(binary_file, 'explore3D_' + '%d' % (q) + '.config', Nodes, Procs, Means, Acceleration, Efficiency)
                                WriteOptimalsNodes(binary_file, 'explore3D_' + '%d' % (q) + '.config', opt_N[0], opt_n[0], "mean time")
                                DisplayOptimals(opt_N, opt_n, "Mean time", binary_file, 'explore3D_' + '%d' % (q) + '.config')
			elif result == "explore2D":		
				WriteDataNodes(binary_file, 'explore2D_' + '%d' % (q) + '.config', Nodes, Procs, Means, Acceleration, Efficiency)
				WriteOptimalsNodes(binary_file, 'explore2D_' + '%d' % (q) + '.config', opt_N[0], opt_n[0], "mean time")
                                DisplayOptimals(opt_N, opt_n, "Mean time", binary_file, 'explore2D_' + '%d' % (q) + '.config')
			elif result == "explore1D":		
				WriteDataNodes(binary_file, 'explore1D_' + '%d' % (q) + '.config', Nodes, Procs, Means, Acceleration, Efficiency)
				WriteOptimalsNodes(binary_file, 'explore1D_' + '%d' % (q) + '.config', opt_N[0], opt_n[0], "mean time")
                                DisplayOptimals(opt_N, opt_n, "Mean time", binary_file, 'explore1D_' + '%d' % (q) + '.config')

			GetOptimalEfficiencyNodes(opt_N, opt_n, Nodes, Procs, Efficiency)
			optimal_procs[q] = opt_n[0]
			optimal_nodes[q] = opt_N[0]
			if result == "explore3D":		
				WriteOptimalsNodes(binary_file, 'explore3D_' + '%d' % (q) + '.config', opt_N[0], opt_n[0], "efficiency")
				DisplayOptimals(opt_N, opt_n, "Efficiency", binary_file, 'explore3D_' + '%d' % (q) + '.config')
			elif result == "explore2D":
				WriteOptimalsNodes(binary_file, 'explore2D_' + '%d' % (q) + '.config', opt_N[0], opt_n[0], "efficiency")
				DisplayOptimals(opt_N, opt_n, "Efficiency", binary_file, 'explore2D_' + '%d' % (q) + '.config')
			elif result == "explore1D":
				WriteOptimalsNodes(binary_file, 'explore1D_' + '%d' % (q) + '.config', opt_N[0], opt_n[0], "efficiency")
				DisplayOptimals(opt_N, opt_n, "Efficiency", binary_file, 'explore1D_' + '%d' % (q) + '.config')
...
def RunConfigFixed(result, path, binary_file, config_file, N, n, Means, number) :
	...
	for q in 1,2,3,4:
                if result == "explore3D":
                        path[0] = "salloc -N " + '%d' % (N) + " -n " + '%d' % (n) + " -p max1hour " + "mpirun --bind-to-core ./" + binary_file + " explore3D_" + '%d' % (q) + '.config'
                        stmt = "Start(path)"
                        Means[q] = GetMean(stmt, number)
                if result == "explore2D":
                        path[0] = "salloc -N " + '%d' % (N) + " -n " + '%d' % (n) + " -p max1hour " + "mpirun --bind-to-core ./" + binary_file + " explore2D_" + '%d' % (q) + '.config'
                        stmt = "Start(path)"
                        Means[q] = GetMean(stmt, number)
                if result == "explore1D":
                        path[0] = "salloc -N " + '%d' % (N) + " -n " + '%d' % (n) + " -p max1hour " + "mpirun --bind-to-core ./" + binary_file + " explore1D_" + '%d' % (q) + '.config'
                        stmt = "Start(path)"
                        Means[q] = GetMean(stmt, number)
...
if len(sys.argv) < 2:
	...
	input_parameters = []
		...
		opt_n = [1]
                ...
                if len(sys.argv) == 5:
                        print "\n>> Will start with fixed mpirun parameters"
		...
		if config_file[0:7] == "explore":
                        ans = raw_input("\n>> Temporary config is also used for config optimization stage. Would you like to run config optimization? ('y' or 'n')? : ")
                        if ans == 'y':
