...
int sum_flag, print_flag, time_flag;
...
double UsingStencils(int length_x, int length_y, int steps) {
  ...
  for (int o = 0; o < repeats; ++o) {
    ...
    for (int t = 0; t < steps; ++t) {
      data(0)(static_cast<int>(kSrcEz), i, j) =
        exp(-static_cast<double>(t * t) / (steps * steps));
...
void PreparePlotDataFile(int length_x, int length_y, int steps) {
  char basename[80] = "results_size_", filename[100];
  ...
  fprintf(f, "#Blocks' length\tStencil\tStencil(blocks)\tRange\tRange(blocks)\tBest");
...
void WritePlotData(int length_x, int length_y, int blocks,
  ...
  FILE *f;
  f = fopen(filename, "a");
  fprintf(f, "%d\t\t%.2f\t%.2f\t\t%.2f\t%.2f\t", blocks, t1, t2, t3, t4);
  double min = t1;
  min = (t2 < min) ? t2:min;
  min = (t3 < min) ? t3:min;
  min = (t4 < min) ? t4:min;
  ...
  fprintf(f, "\t%.2e\t%.2e\t%.2e\t%.2e\n", (t1 / total), (t2 / total), (t3 / total), (t4 / total));
  fclose(f);
}
