...
class FDTD {
  public:
  int length_x_, length_y_, steps_;
  double ****data_;
  FDTD(int length_x, int length_y, int steps, int repeats);
  ~FDTD();
...
FDTD::FDTD(int length_x, int length_y, int steps, int repeats) {
  length_x_ = length_x;
  length_y_ = length_y;
  steps_ = steps;
...
void FDTD::UpdateFields2DStdC(int x1, int x2, int y1, int y2) {
  for (int i3 = x1; i3 <= x2; ++i3) {
    for (int i4 = y1; i4 <= y2; ++i4) {
      data_[1][kHx][i3][i4] = data_[0][kChxh][i3][i4] *
        data_[0][kHx][i3][i4] - data_[0][kChxe][i3][i4] *
        (data_[0][kEz][i3][i4 + 1] - data_[0][kEz][i3][i4]);
    }
  }

  for (int i3 = x1; i3 <= x2; ++i3) {
    for (int i4 = y1; i4 <= y2; ++i4) {
      data_[1][kHy][i3][i4] = data_[0][kChyh][i3][i4] *
        data_[0][kHy][i3][i4] + data_[0][kChye][i3][i4] *
        (data_[0][kEz][i3 + 1][i4] - data_[0][kEz][i3][i4]);
    }
  }

  for (int i3 = x1; i3 <= x2; ++i3) {
    for (int i4 = y1; i4 <= y2; ++i4) {
      data_[1][kEz][i3][i4] = data_[0][kCeze][i3][i4] *
        data_[0][kEz][i3][i4] + data_[0][kCezh][i3][i4] *
        ((data_[0][kHy][i3][i4] - data_[0][kHy][i3 - 1][i4])
         - (data_[0][kHx][i3][i4] - data_[0][kHx][i3][i4 - 1]))
        + data_[0][kSrcEz][i3][i4];
    }
  }
}
  ...
  for (int i1 = 0; i1 < depth; ++i1) {
    for (int i3 = 0; i3 <= length_x_ + 1; ++i3) {
      for (int i4 = 0; i4 <= length_y_ + 1; ++i4) {
        data_[i1][kEz][i3][i4] = initial;
        data_[i1][kHx][i3][i4] = initial;
        data_[i1][kHy][i3][i4] = initial;
        data_[i1][kSrcEz][i3][i4] = initial;
      }
    }
  }
}
...
void FDTD::SetCoefficients() {
  for (int i1 = 0; i1 < depth; ++i1) {
    for (int i3 = 0; i3 <= length_x_ + 1; ++i3) {
      for (int i4 = 0; i4 <= length_y_+ 1; ++i4) {
        data_[i1][kCeze][i3][i4] = 1;
        data_[i1][kCezh][i3][i4] = coefficient;
        data_[i1][kChyh][i3][i4] = coefficient;
        data_[i1][kChye][i3][i4] = coefficient;
        data_[i1][kChxh][i3][i4] = coefficient;
        data_[i1][kChxe][i3][i4] = coefficient;
      }
    }
  }
}
...
void FDTD::CycleArrays() {
  for (int i2 = 0; i2 < components; ++i2) {
    for (int i3 = 0; i3 <= length_x_ + 1; ++i3) {
      for (int i4 = 0; i4 <= length_y_ + 1; ++i4) {
        data_[0][i2][i3][i4] = data_[1][i2][i3][i4];
      }
    }
  }
}
...
double FDTD::UsingStdC() {
  ...
  for (int o = 0; o < repeats; ++o) {
    ...
    for (int t = 0; t < steps_; ++t) {
      ...
      UpdateFields2DStdC(1, length_x_, 1, length_y_);
      CycleArrays();
    }
    end = MPI_Wtime();
    time += end - start;
...
double FDTD::UsingStdCBlocks(int block_length) {
    ...
    for (int t = 0; t < steps_; ++t) {
      ...
      for (int v1 = 1; v1 <= lim; ++v1) {
        for (int v2 = 1; v2 <= lim; ++v2) {
          UpdateFields2DStdC(Block_ranges_x1[v1],
                             Block_ranges_x2[v1],
                             Block_ranges_y1[v2],
                             Block_ranges_y2[v2]);
        }
  ...
  if (time_flag == 1) {
    printf("\n>> StdC 2D(%d blocks) took %f \n", lim,
           static_cast<float>(time / repeats));
  }
  if (sum_flag == 1) {
    printf("\n>> StdC 2D (%d blocks) Ez control sum is %e\n",  lim, EzControlSum());
  }
  if (print_flag == 1) {
    PrintField();
  }
...
void RunCalculation() {
  ...
  int C = max_size * max_size * max_steps;
  ...
  for (int size = min_size; size <= max_size; size *= 2) {
    ...
    FDTD task(length_x, length_y, steps, repeats);
    ...
    if (sm_min < max) {
      ...
      for (int i2 = 0; i2 < counter2 - 1; ++i2) {
        WritePlotData(length_x, length_y, blocks[i2],
