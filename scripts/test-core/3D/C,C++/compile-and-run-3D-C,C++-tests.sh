...
##############################
	...
	echo ">> Launches 3D C/C++ tests with blocks option"
	cd $path_blitz
	...
	echo ">> Done running 3D C/C++ tests with blocks option"
	##############################
	elif [[ $mode == "b" ]]; then
	...
	echo ">> Launches 3D C/C++ tests with blocks option and valgrind profiling for 0.1x timesteps"
	cd $path_blitz
	mpic++ $blitz -O2 -Dblk=1 -DCLS=$(getconf LEVEL1_DCACHE_LINESIZE) -ftemplate-depth-30 -o $blitz.bin
	./$blitz.bin
	cd $path_c
	...
	echo ">> Done running 3D C/C++ tests with blocks option and valgrind profiling for 0.1x timesteps"
	##############################
	else
	echo ">> Unknown mode!"
	fi
